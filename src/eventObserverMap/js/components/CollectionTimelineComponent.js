...
const cleanData = eventData => {
  let units = eventData.filter((obj) => obj.date !== '');
  units.forEach((d) => {
    const formatedDate = moment(d.date).format('YYYY-MM-DD');
    if (formatedDate === 'Invalid date') {
      d.date = moment(d.date, 'DD-MM-YYYY').format('YYYY-MM-DD'); // eslint-disable-line
    } else {
      d.date = formatedDate; // eslint-disable-line
    }
  });
...
export default class CollectionTimelineComponent extends Component {
  ...
  componentDidUpdate() {
    // exclude events don't have dates
